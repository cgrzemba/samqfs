	...
	void *vcmd)
{
	...
	"above high"};
	sam_fs_fifo_t *command = (sam_fs_fifo_t *)vcmd;
	fs_wmstate_t *wmstate_cmd = &command->param.fs_wmstate;
	dev_ent_t *device;
	dev_ptr_tbl_t *dev_ptr_tbl;
	equ_t	fseq;

	fseq = wmstate_cmd->fseq;
	prv_fswm_priority(fseq, wmstate_cmd->wmstate);
	dev_ptr_tbl = (dev_ptr_tbl_t *)SHM_REF_ADDR(
	    ((shm_ptr_tbl_t *)master_shm.shared_memory)->dev_table);
	device = (dev_ent_t *)SHM_REF_ADDR((int)dev_ptr_tbl->d_ent[fseq]);
	if (device && wmstate_cmd->wmstate < FS_WMSTATE_MAX) {
		if (DBG_LVL(SAM_DBG_DEBUG)) {
			sam_syslog(LOG_DEBUG, "%s - %s watermark", device->set,
			    wm_str[wmstate_cmd->wmstate]);
		}
	}

	/*
	 * The releaser message coexists with the watermark state message at
	 * this time.
	 */
	free(vcmd);
	thr_exit(NULL);
