/*
 *    SAM-QFS_notice_begin
 *
 * CDDL HEADER START
 *
 * The contents of this file are subject to the terms of the
 * Common Development and Distribution License (the "License")
 * You may not use this file except in compliance with the License.
 *
 * You can obtain a copy of the license at usr/src/OPENSOLARIS.LICENSE
 * or http://www.opensolaris.org/os/licensing.
 * See the License for the specific language governing permissions
 * and limitations under the License.
 *
 * When distributing Covered Code, include this CDDL HEADER in each
 * file and include the License file at usr/src/OPENSOLARIS.LICENSE.
 * If applicable, add the following below this CDDL HEADER, with the
 * fields enclosed by brackets "[]" replaced with your own identifying
 * information: Portions Copyright [yyyy] [name of copyright owner]
 *
 * CDDL HEADER END
 */
/*
 * Copyright 2008 Sun Microsystems, Inc.  All rights reserved.
 * Use is subject to license terms.
 *
 *    SAM-QFS_notice_end
 */

// ident	$Id: ArchivePolCriteriaCopy.java,v 1.3 2008/03/17 14:43:44 am143972 Exp $

package com.sun.netstorage.samqfs.web.model.archive;

public interface ArchivePolCriteriaCopy {

    public ArchivePolCriteria getArchivePolCriteria();

    public void setArchivePolCriteria(ArchivePolCriteria criteria);

    public int getArchivePolCriteriaCopyNumber();

    public long getArchiveAge();

    public void setArchiveAge(long age);

    public int getArchiveAgeUnit();

    public void setArchiveAgeUnit(int unit);

    public long getUnarchiveAge();

    public void setUnarchiveAge(long age);

    public int getUnarchiveAgeUnit();

    public void setUnarchiveAgeUnit(int ageUnit);

    public boolean isRelease();

    public void setRelease(boolean release);

    public boolean isNoRelease();

    public void setNoRelease(boolean noRelease);
}
