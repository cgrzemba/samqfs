...
// ident	$Id: vsnpools.js,v 1.9 2008/03/17 14:40:25 am143972 Exp $

var POPUP_URI = "/archive/NewEditVSNPool";

function getServerKey(field) {
    if (field.form.name == "VSNPoolSummaryForm") {
        return document.VSNPoolSummaryForm.elements[
            "VSNPoolSummary.ServerName"].value;
    } else {
        return document.VSNPoolDetailsForm.elements[
        "VSNPoolDetails.ServerName"].value;
    }
}
...
function handlePoolSelection(field) {
  ...
  if (field != null && isTableDeselectButton(field)) {
    ...
    if (!isPoolInUse(field.value)) {
      ...
      // make sure to disable pool if selected after selecting a pool thats not
      // in use
      ccSetButtonDisabled(deleteButton, formName, 1);
    }
