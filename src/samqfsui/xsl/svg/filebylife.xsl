...
<!--  SAM-QFS_notice_end                                                  -->
<!--                                                                      -->
<!-- $Id: filebylife.xsl,v 1.16 2008/12/16 00:12:31 am143972 Exp $ -->
...
<xsl:stylesheet 
    ...
    <xsl:template match="/MetricReport">
        <svg width="700" height="700">
            ...
            <xsl:call-template name="display-legend">
                <xsl:with-param name="name" select="string('filebylife')"/>
            </xsl:call-template>
            <!-- display graph -->
    ...
    <xsl:template match="filesystem">
        ...
        <!-- total length of x-Axis = 480 -->
        <!-- x interval is calculated based on startdate and endDate elements -->
        ...
        <xsl:choose>
            <xsl:when test="$total &gt; 0">
                ...
                <!-- the 6 graphs are drawn using polyline, the x/y coord are -->
                ...
                <!-- graph 6 -->
                <xsl:text disable-output-escaping="yes">&lt;polyline points='</xsl:text>	
                <xsl:for-each select="date/range[@val= '0']">
                    <xsl:sort select="ancestor::date/@val" order="ascending" data-type="number"/>
                    <xsl:call-template name="fill-coords">
                        <xsl:with-param name="currentDate" select="ancestor::date/@val"/>
                    </xsl:call-template>
                ...
                <xsl:text disable-output-escaping="yes">
                ' fill='none' stroke='#0000FF' stroke-width='2' marker-start='url(#marker-0000FF)' marker-mid='url(#marker-0000FF)' marker-end='url(#marker-0000FF)'</xsl:text>
                <xsl:text disable-output-escaping="yes">/&gt;</xsl:text>
    ...
    <xsl:template name="display-title">
        <text style="font-family:arial; font-weight:bold; font-size:16; fill:#666666; letter-spacing:1.5; text-anchor:middle;" 
              x="310" y="20">
            <xsl:value-of select="string(SamUtil:getResourceString('reports.title.type.filebylife'))"/>
        </text>
    </xsl:template>
    ...
    <xsl:template name="display-subtitle">
        <text style="font-family:arial; font-weight:normal; font-size:12; letter-spacing:1.5; text-anchor:middle;" 
              x="310" y="40">
            <xsl:value-of select="string(SamUtil:getResourceString('reports.subtitle.type.filebylife'))"/>
        </text>
    </xsl:template>
    ...
    <xsl:template name="display-yaxis-label">
        ...
        <g transform="rotate(-90)">
            <text style="font-family:arial; font-weight:bold; font-size:12; letter-spacing:1.5; fill:#666666;">
                <xsl:value-of select="string(SamUtil:getResourceString('reports.filedistribution.filebylife.yaxistext'))"/>
            </text>
        </g>
