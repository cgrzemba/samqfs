...
<!--  SAM-QFS_notice_end                                                  -->
<!--                                                                      -->
<!-- $Id: filebyowner.xsl,v 1.12 2008/03/17 14:44:04 am143972 Exp $ -->
...
<xsl:stylesheet 
    ...
    <xsl:template name="display-title">
        <text style="font-family:arial; font-weight:bold; font-size:16; fill:#666666; letter-spacing:1.5; text-anchor:middle;" 
              x="310" y="20">
            <xsl:value-of select="string(SamUtil:getResourceString('reports.title.type.filebyowner'))"/>
        </text>
    </xsl:template>
    ...
    <xsl:template name="display-subtitle">
        <text style="font-family:arial; font-weight:normal; font-size:12; letter-spacing:1.5; text-anchor:middle;" 
              x="310" y="40">
            <xsl:value-of select="string(SamUtil:getResourceString('reports.subtitle.type.filebyowner'))"/>
        </text>
    </xsl:template>
    ...
    <xsl:template name="display-yaxis-label">
        ...
        <g transform="rotate(-90)">
            <text style="font-family:arial; font-weight:bold; font-size:12; letter-spacing:1.5; fill:#666666;">
                <xsl:value-of select="string(SamUtil:getResourceString('reports.filedistribution.filebyowner.yaxistext'))"/>
            </text>
        </g>
