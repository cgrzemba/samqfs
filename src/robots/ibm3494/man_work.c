	...
	void *vlibrary)
{
	...
	for (;;) {
		mutex_lock(&library->list_mutex);	/* get the list lock */

		/*
		 * See if there in anything to do. We will wait
		 * if the active count is 0, or its equal to the same
		 * value it had when we last woke up and there is a
		 * delayed request.
		 */
		...
		if (auto_check <= now && !library->un->status.b.scanning) {
		...
		do {
			...
			ETRACE((LOG_NOTICE, "LbEv c %#x n %#x (%d)", current,
			...
			case EVENT_TYPE_MESS:
				...
				case MESS_CMD_STATE:
					/*
					 * state_request will put event back on
					 * free list when done
					 */
				...
				case MESS_CMD_TAPEALERT:
					/*
					 * tapealert_solicit will put event
					 * back on free list when done
					 */
				...
				case MESS_CMD_SEF:
					/*
					 * sef_solicit will put  event back on
				...
				case MESS_CMD_MOUNT:
					/*
					 * mount_request will put
					 * event back on free list when done
					 */
				...
				case MESS_CMD_AUDIT:
		/* Incorrect indentation for cstyle changes */
		if (start_audit(library, current, current->
		    request.message.param.audit_request.slot)) {
				...
				case MESS_CMD_UNLOAD:
					/*
					 * unload_request will put event back
					 * on the free list when  done.
					 * Unload request will add the request
				...
				case MESS_CMD_CLEAN:
					/*
					 * clean_request will put event back on
					 * free list when the command is done.
					 * clean_request will add the request
					 * to the drives worklist.
					 */
				...
				case MESS_CMD_TODO:
					/*
					 * todo_request will put event back on
					 * free list when the command is done.
					 */
				...
				case MESS_CMD_ADD:
					/*
					 * add_to_cat_req will put event back on
					 * the free list when done
					 */
				...
				case MESS_CMD_EXPORT:
					/*
					 * export_request will put event back
					 * on the free list when done
					 * export_request will add request to
					 * the mail-box worklist.
					 */
			...
			break;		/* if we precessed an entry, break */
		} while (current != NULL);
	}
}
