/*
 *	main.c - main routine for the stk library.
 */
...
main(int argc, char **argv)
{
	...
	sprintf(logname, "stk-%d", library->eq);
	...
	if (DBG_LVL(SAM_DBG_DEBUG)) {
		if ((sizeof (struct sam_message_s) - MESS_HEADER_SIZE) <
				sizeof (struct stk_resp_acs)) {
			sam_syslog(LOG_ERR,
				"Response(%#x) won't fit in message (%#x).",
				sizeof (struct stk_resp_acs),
				(sizeof (struct sam_message_s) -
							MESS_HEADER_SIZE));
			exit(1);
		}
		if (sizeof (DRIVEID) != sizeof (uint_t)) {
			sam_syslog(LOG_ERR,
			    "sizeof (DRIVEID) != sizeof (uint)");
			exit(1);
		}
	}
	...
	/* LINTED pointer cast may result in improper alignment */
	library->help_msg =
	    (stk_priv_mess_t *)SHM_REF_ADDR(library->un->dt.rb.private);
	...
	if (DBG_LVL(SAM_DBG_RBDELAY)) {
		...
		while (ldk > 0 && DBG_LVL(SAM_DBG_RBDELAY)) {
			sprintf(lc_mess,
			    "waiting for %d seconds pid %d", ldk, mypid);
			sleep(10);
			ldk -= 10;
		}
		*lc_mess = '\0';
	...
	if (thr_create(NULL, MD_THR_STK, manage_list, (void *)library,
	    (THR_BOUND | THR_NEW_LWP | THR_DETACHED),
	    &threads[STK_WORK_THREAD])) {

		sam_syslog(LOG_ERR,
		    "Unable to start thread manage_list: %m.\n");
		thr_kill(threads[STK_MSG_THREAD], SIGINT);
		thr_exit(NULL);
	}
	 ...
	 * indication of shutdown. This is accomplished using the
	 * signals SIGALRM, SIGINT, and SIGTERM.  This is not
	...
	for (;;) {
		...
		case SIGALRM:
			if ((threads[STK_MSG_THREAD] == (thread_t)-1) ||
			    (threads[STK_WORK_THREAD] == (thread_t)-1)) {
				/*
				 * If any processing threads have disappeared,
			 ...
			 * 3) kill the helper pids if there are any
		...
		case SIGTERM:
			...
			sam_syslog(LOG_INFO, "%s: Shutdown by signal %d",
			    ent_pnt, what_signal);
			if (library->helper_pid > 0)
				kill(library->helper_pid, 9);
			if (library->ssi_pid > 0)
