#include "acssys.h"
#ifndef lint
static char SccsId[] = "@(#) %full_name:  acsapi/csrc/acs_query_cl/2.01A %";
#endif
/*
 *
 ...
 *      query_clean request to the ACSSS software. A QUERY CLEAN request
 ...
 *  volId          - Array of ids of cleaning cartridges to be queried.
 *  count          - Number of cleaning cartridges to be queried. 
 *                   If count = 0, all cleaning cartridges are queried.
 *
 * Implicit Inputs:
 *
 * Implicit Outputs:
...
) 
{
    ...
    ACSMESSAGES msg_num;
    STATUS acsReturn;
    QUERY_REQUEST queryCleanRequest;
	...
	acsReturn = acs_build_header ((char *) &queryCleanRequest,
	...
	if (acsReturn == STATUS_SUCCESS) {
	    queryCleanRequest.type = TYPE_CLEAN;
	    queryCleanRequest.select_criteria.vol_criteria.volume_count =
		count;
		...
		for (i = 0; i < count; i++) {
		    queryCleanRequest.select_criteria.vol_criteria.volume_id[i] =
			volId[i];
		}
		acsReturn = acs_send_request (&queryCleanRequest,
		    sizeof (QUERY_REQUEST));
	    }
	}
    }
