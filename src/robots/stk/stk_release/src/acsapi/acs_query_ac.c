#include "acssys.h"
#ifndef lint
static char SccsId[] = "@(#) %full_name:  acsapi/csrc/acs_query_ac/2.01A %";
#endif
/*
 *
 ...
 *      query_acs request to the ACSSS software.  A QUERY ACS request
 ...
 *  acs            - Array of ids of ACSs to be queried.
 *  count          - Number of ACSs to be queried.
 *
 * Implicit Inputs:
 *
 * Implicit Outputs:
...
) 
{
	...
	acsReturn = acs_build_header ((char *) &queryACSRequest,
	...
	if (acsReturn == STATUS_SUCCESS) {
	    queryACSRequest.select_criteria.acs_criteria.acs_count = count;
	    queryACSRequest.type = TYPE_ACS;
		...
		for (i = 0; i < count; i++) {
		    queryACSRequest.select_criteria.acs_criteria.acs_id[i] =
			acs[i];
		}
		acsReturn = acs_send_request (&queryACSRequest, sizeof (QUERY_REQUEST));
	    }
	}
    }
