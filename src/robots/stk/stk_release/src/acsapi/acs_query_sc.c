#include "acssys.h"
#ifndef lint
static char SccsId[] = "@(#) %full_name:  acsapi/csrc/acs_query_sc/2.01A %";
#endif
/*
 ...
 *      query_scratch request to the ACSSS software.  A QUERY SCRATCH
 *      request packet is constructed (using the parameters given) and
 *      sent to the SSI process. A QUERY SCRATCH request retrieves s
 *      list of scratch volumes for the scratch pools specified.
 *
 * Return Values:
...
) 
{
    ...
    ACSMESSAGES msg_num;
    STATUS acsReturn;
    QUERY_REQUEST queryScratchRequest;
	...
	acsReturn = acs_build_header ((char *) &queryScratchRequest,
	...
	if (acsReturn == STATUS_SUCCESS) {
	    queryScratchRequest.type = TYPE_SCRATCH;
	    queryScratchRequest.select_criteria.pool_criteria.pool_count
		= count;
	    if (count > MAX_ID) {
		...
		for (i = 0; i < count; i++) {
		    queryScratchRequest.select_criteria.pool_criteria.
			pool_id[i].pool = pool[i];
		}
		acsReturn = acs_send_request (&queryScratchRequest, sizeof (QUERY_REQUEST));
	    }
	}
    }
