#include "acssys.h"
#ifndef lint
static char SccsId[] = "@(#) %full_name:  acsapi/csrc/acs_query_vo/2.01A %";
#endif
/*
 *
 ...
 *      query_volume request to the ACSSS software.  A QUERY VOLUME
 ...
 *                   If count = 0, all tape cartridges are queried.
 *
 * Implicit Inputs:
 *
 * Implicit Outputs:
...
) 
{
    ...
    ACSMESSAGES msg_num;
    STATUS acsReturn;
    QUERY_REQUEST queryVolumeRequest;
	...
	acsReturn = acs_build_header ((char *) &queryVolumeRequest,
	...
	if (acsReturn == STATUS_SUCCESS) {
	    queryVolumeRequest.type = TYPE_VOLUME;
	    queryVolumeRequest.select_criteria.vol_criteria.volume_count
		= count;
	    if (count > MAX_ID) {
		...
		for (i = 0; i < count; i++) {
		    queryVolumeRequest.select_criteria.vol_criteria.
			volume_id[i] = vol_id[i];
		}
		acsReturn = acs_send_request (&queryVolumeRequest,
		    sizeof (QUERY_REQUEST));
	    }
	}
    }
