#include "acssys.h"
#ifndef lint
static char SccsId[] = "@(#) %full_name:  acsapi/csrc/acs_clear_dr/2.01A %";
#endif
/*
 *
 ...
 *      clear_lock request to the ACSSS software.  A CLEAR_LOCK DRIVE
 ...
 *  driveId[MAX_ID]  - An array of ids of drives to be locked.
...
#undef SELF
#define SELF "acs_clear_lock_drive"
#undef ACSMOD
#define ACSMOD 5
...
) 
{
    ...
    clearLockRequest.type = TYPE_DRIVE;
	...
	for (index = 0; index < count; index++) {
	    clearLockRequest.identifier.drive_id[index] = driveId[index];
	}
	acsReturn = acs_send_request (&clearLockRequest,
	    sizeof (CLEAR_LOCK_REQUEST));
    }
