#include "acssys.h"
#ifndef lint
static char SccsId[] = "@(#) %full_name:  acsapi/csrc/acs_query_po/2.01A %";
#endif
/*
 *
 ...
 *      query_port request to the ACSSS software.  A QUERY PORT request
 ...
 *  portId         - Array of ids of pass thru ports to be queried.
 *  count          - The number of pass thru ports to be queried.
 *                   If count = 0, all pass thru ports are queried.
 *
 *
 * Implicit Inputs:
 *
...
) 
{
    ...
    ACSMESSAGES msg_num;
    STATUS acsReturn;
    QUERY_REQUEST queryPortRequest;
	...
	acsReturn = acs_build_header ((char *) &queryPortRequest,
	...
	if (acsReturn == STATUS_SUCCESS) {
	    queryPortRequest.type = TYPE_PORT;
	    queryPortRequest.select_criteria.port_criteria.port_count =
		count;
		...
		for (i = 0; i < count; i++) {
		    queryPortRequest.select_criteria.port_criteria.
			port_id[i] = portId[i];
		}
		acsReturn = acs_send_request (&queryPortRequest,
		    sizeof (QUERY_REQUEST));
	    }
	}
    }
