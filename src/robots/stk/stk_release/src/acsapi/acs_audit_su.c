#include "acssys.h"
#ifndef lint
static char SccsId[] = "@(#) %full_name:  acsapi/csrc/acs_audit_su/2.01A %";
#endif
/*
 *
 ...
 *      audit_subpanel request to the ACSSS software.  A AUDIT SUBPANEL
 ...
 *  subpanelId[MAX_ID]  - An array of ids of subpanels to be audited.
 *  capId          - The id of the CAP used for ejection of cartridges.
 *  count          - The number of ids in the subpanel id array.
 *
 * Implicit Inputs:
 *
 * Implicit Outputs:
...
#undef SELF
#define SELF "acs_audit_subpanel"
#undef ACSMOD
#define ACSMOD 3
...
) 
{
    ...
    ACSMESSAGES msg_num;
    STATUS acsReturn;
    AUDIT_REQUEST auditSubpanelRequest;
	...
	acsReturn = acs_build_header ((char *) &auditSubpanelRequest,
	...
	if (acsReturn == STATUS_SUCCESS) {
	    auditSubpanelRequest.type = TYPE_SUBPANEL;
	    auditSubpanelRequest.cap_id = capId;
	    auditSubpanelRequest.count = count;
	    if (count > MAX_ID) {
		...
		for (index = 0; index < count; index++) {
		    auditSubpanelRequest.identifier.subpanel_id[index]
			= subpanelId[index];
		}
		acsReturn = acs_send_request (&auditSubpanelRequest,
		    sizeof (AUDIT_REQUEST));
	    }
	}
    }
