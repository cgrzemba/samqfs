#include "acssys.h"
#ifndef lint
static char SccsId[] = "@(#) %full_name:  acsapi/csrc/acs_lock_dr/2.01A %";
#endif
/*
 *
 ...
 *      lock request to the ACSSS software.  A LOCK DRIVE request packet
 ...
 *  lockId        - Id of the lock on the drive or cartridge.
 *  userId        - User identifier associated with the lock.
 *  driveId       - Array of ids of drives to be locked.
...
) 
{
	...
	if (acsReturn == STATUS_SUCCESS) {
		...
		for (index = 0; index < count; index++) {
		    lockRequest.identifier.drive_id[index] = driveId[index];
		}
