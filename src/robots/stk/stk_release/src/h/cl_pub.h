 ...
 *
 *   functional description of objects defined in header file.  definitions
 *   and declaration should be in alphabetic order for readability.  This file
 *   contains the public interface to the common library routines.  Feature
 *   specific routines are in cl_FEATURE_pub.h.
 *
 * Modified by:
 *
 *   Alec Sharp				Original.
 *   Andy Steere	30-Jul-93	R5.0 added cl_mk_path and cl_file_line
 *   Andy Steere	 1-Aug-93	R5.0 added stdio.h to include list,
 *					removed const from cl_mk_path.
 *   Janet Borzuchowski 11-Aug-93	R5.0 added cl_mm_info declaration.
 *					This was moved from cl_mm_pub.h because
 *					of header contention problems.
 *   Alec Sharp         20-Aug-93	R5.0 added cl_expand_string
 *   Tariq Khawaja      26-Aug-93	R5.0 added cl_mt_vt
 *   Alec Sharp         07-Sep-93       R5.0 added cl_set_hostid
 *   Alec Sharp         24-Sep-93       R5.0 added several functions for
 *                                      message logging and registration of
 *                                      logging functions.
 *   Alec Sharp         03-Nov-93       R5.0 added cl_str_to_buf, cl_sig_desc
 *   David Farmer       24-Nov-1993     removed cl_log_event
 *   Alec Sharp	 	01-Dec-1993	Removed cl_message
 *   Scott Siao         21-Oct-2001     Added cl_resource_event
 */
...
STATUS  cl_caps_in_lsm(int panel_count, PANEL_TYPE panel_types[MAX_PANEL + 1],
...
char    *cl_mk_path( char * sub_dir_path, char *filename, char *new_path);
...
STATUS  cl_chk_offline(TYPE type, IDENTIFIER *identifier, BOOLEAN initial);
STATUS  cl_cln_read(QUERY_TYPE query_type, VOLUME_RECORD *volume_record);
...
STATUS  cl_drv_write(DRIVE_RECORD *drive_record, WRITE_MODE write_mode);
STATUS  cl_loc_read(QUERY_TYPE query_type, LOCKID_RECORD *lockid_record);
...
STATUS  cl_vol_read(QUERY_TYPE query_type, VOLUME_RECORD *volume_record);
STATUS  cl_vol_update(VOLID vol_id, FIELD field_id,
		      int cur_value, int new_value, LOCKID lock_id,
		      REQUEST_HEADER *p_req_hdr);
STATUS  cl_vol_write(VOLUME_RECORD *volume_record, WRITE_MODE write_mode);
