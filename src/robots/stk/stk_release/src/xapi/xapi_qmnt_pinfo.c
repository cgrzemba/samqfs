...
/** All rights reserved.                                             */
/**                                                                  */
/** File Name:      xapi_qmnt_pinfo.c                                */
/** Description:    XAPI QUERY MOUNT PINFO processor.                */
/**                                                                  */
...
/** FUNCTION PROLOGUE ************************************************/
/**                                                                  */
/** Function Name: xapi_qmnt_pinfo                                   */
/** Description:   The XAPI QUERY MOUNT PINFO processor.             */
/**                                                                  */
/** Return a drive list of eligible drives for the specified         */
/** POOLID (subpool index), media, and management class.             */
/**                                                                  */
/** The QUERY MOUNT PINFO request is processed by calling the XAPI   */
/** client query mount scratch service (xapi_qmnt_one).              */
/**                                                                  */
/** If multiple POOLIDs (subpool indexes) are specified, then the    */
/** request is treated as multiple single POOLID requests (all       */
/** with the same media type code and management class name).        */
...
/**                                                                  */
/** The QUERY MOUNT PINFO command is allowed to proceed even when    */
/** the XAPI client is in the IDLE state.                            */
/**                                                                  */
/**==================================================================*/
/** The ACSAPI QUERY MOUNT SCRATCH PINFO request consists of:        */
/**==================================================================*/
...
/**      iv.  mgmt_clas (management class name string)               */
/**                                                                  */
/**==================================================================*/
/** The ACSAPI QUERY MOUNT SCRATCH PINFO response consists of:       */
/**==================================================================*/
                           ...
                           struct XAPIREQE *pXapireqe)
{
    ...
    int                 lastRC;
    int                 qpinfoResponseSize;
    int                 i;
    int                 j;

    QUERY_REQUEST      *pQuery_Request      = (QUERY_REQUEST*) pXapireqe->pAcsapiBuffer;
    QU_MSC_PINFO_CRITERIA *pQu_Msc_Pinfo_Criteria = 
    &(pQuery_Request->select_criteria.mount_scratch_pinfo_criteria);

    int                 poolCount           = pQu_Msc_Pinfo_Criteria->pool_count;
    ...
    /* response.  However that would cause the QUERY_MOUNT_SCRATCH   */
    ...
    /*****************************************************************/
    qpinfoResponseSize = (char*) pQu_Msc_Status -
    ...
    TRMSGI(TRCI_XAPI,
           "Entered; QUERY MOUNT PINFO request=%08X, size=%d, "
           "count=%d, MAX_ID=%d, MAX_MESSAGE_SIZE=%d, "
           "QUERY MOUNT PINFO response=%08X, size=%d\n",
    ...
    /*****************************************************************/
    /* NOTE: The ACSAPI QUERY MOUNT PINFO request allows multiple    */
        ...
        i++)
    {
        ...
        pQu_Msc_Status->pool_id.pool = pQu_Msc_Pinfo_Criteria->pool_id[i].pool;
        subpoolIndex = (short) pQu_Msc_Pinfo_Criteria->pool_id[i].pool;
        ...
        {
            acsapiMediaType = (char) pQu_Msc_Pinfo_Criteria->media_type;
            ...
            if (pQu_Msc_Pinfo_Criteria->mgmt_clas.mgmt_clas[0] > ' ')
            {
                pMgmtClassString = &(pQu_Msc_Pinfo_Criteria->mgmt_clas.mgmt_clas[0]);
            }
