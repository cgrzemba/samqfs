...
/** All rights reserved.                                             */
/**                                                                  */
/** File Name:      xapi_unlock_vol.c                                */
/** Description:    XAPI UNLOCK VOLUME processor.                    */
/**                                                                  */
/**                 Unlock the specified VOLUME(s) from the          */
/**                 specified LOCKID.                                */
/**                                                                  */
/** Change History:                                                  */
/**==================================================================*/
...
/** FUNCTION PROLOGUE ************************************************/
/**                                                                  */
/** Function Name: xapi_unlock_vol                                   */
/** Description:   XAPI UNLOCK VOLUME processor.                     */
/**                                                                  */
/** Unlock the specified VOLUME(s) from the specified (i.e.          */
/** already existing) LOCKID. A LOCKID is simply a number            */
/** between 1 and 32767.                                             */
/**                                                                  */
/** VOLUME LOCK(s) are managed on the server.  The ACSAPI format     */
/** UNLOCK VOLUME request is translated into an XAPI XML format      */
/** <unlock_volume> request; the XAPI XML request is then            */
/** transmitted to the server via TCP/IP;  The received XAPI XML     */
/** response is then translated into the ACSAPI UNLOCK VOLUME        */
/** response.                                                        */
/**                                                                  */
/** The UNLOCK VOLUME command is NOT allowed to proceed when         */
/** the XAPI client is in the IDLE state.                            */
/**                                                                  */
/** END PROLOGUE *****************************************************/
#undef SELF
#define SELF "xapi_unlock_vol"
                           ...
                           struct XAPIREQE *pXapireqe)
{
    ...
    TRMSGI(TRCI_XAPI,
           "Entered; UNLOCK VOLUME request=%08X, size=%d, count=%d, "
           "MAX_ID=%d, UNLOCK VOLUME response=%08X, size=%d\n",
    ...
    /*****************************************************************/
    /* Now generate the UNLOCK VOLUME ACSAPI response.               */
    /*****************************************************************/
...
/** Description:   Build an XAPI <unlock_volume> request.            */
/**                                                                  */
/** Convert the ACSAPI format UNLOCK VOLUME request into an          */
/** XAPI XML format <unlock_volume> request.                         */
/**                                                                  */
/**==================================================================*/
/** The ACSAPI UNLOCK VOLUME request consists of:                    */
/**==================================================================*/
/** 1.   REQUEST_HEADER data consisting of:                          */
/**      i.   IPC_HEADER                                             */
/**      ii.  MESSAGE_HEADER, and the                                */
/** 2.   UNLOCK VOLUME data consisting of:                           */
/**      i.   type                                                   */
/**      ii.  count                                                  */
...
/**                                                                  */
/**==================================================================*/
/** The XAPI XML <unlock_volume> request consists of:                */
/**==================================================================*/
/** <libtrans>COMMAND                                                */
/**   <header>                                                       */
                                ...
                                int             *pXapiBufferSize)
{
    ...
    VOLID              *pVolid              = &(pUnlock_Request->identifier.vol_id[0]);
    ...
    TRMSGI(TRCI_XAPI,
           "Entered; unlockCount=%d, lockId=%d\n",
           unlockCount,
           lockId);
    ...
    *pXapiBufferSize = 0;
    ...
    if (unlockCount > 0)
    {
        for (i = 0, pVolid = &(pUnlock_Request->identifier.vol_id[0]);
            i < unlockCount;
            i++, pVolid++)
        {
...
/** Description:   Extract the <unlock_volume_request> response.     */
/**                                                                  */
/** Parse the response of the XAPI XML <unlock_volume>               */
/** request and update the appropriate fields of the                 */
/** ACSAPI UNLOCK VOLUME response.                                   */
/**                                                                  */
/**==================================================================*/
/** The XAPI XML <unlock_volume> response consists of:               */
/**==================================================================*/
/** <libreply>                                                       */
/**   <unlock_volume_request>                                        */
/**     <header>                                                     */
/**       header element children                                    */
/**     </header>                                                    */
...
/** </libreply>                                                      */
/**                                                                  */
/** NOTE: Possible <unlock_volume_request><status> values are:       */
...
/**                                                                  */
/**==================================================================*/
/** The ACSAPI UNLOCK VOLUME response consists of:                   */
/**==================================================================*/
                                 ...
                                 struct XMLPARSE *pXmlparse)
{
    ...
    LO_VOL_STATUS      *pLo_Vol_Status      = &(pUnlock_Response->identifier_status.volume_status[0]);
    ...
    while (1)
    {
        ...
        unlockResponseSize = (char*) pLo_Vol_Status -
                             (char*) pUnlock_Response +
                             ((sizeof(LO_VOL_STATUS)) * unlockPacketCount);
        ...
        pLo_Vol_Status = &(pUnlock_Response->identifier_status.volume_status[0]);
