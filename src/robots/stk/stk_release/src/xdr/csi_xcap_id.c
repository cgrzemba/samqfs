static char SccsId[] = "@(#)csi_xcap_id.c	5.3 11/9/93 ";
/*
 * Copyright (1989, 2011) Oracle and/or its affiliates.  All rights reserved.
 *
 * Name:
 *
 *      csi_xcap_id()
 *
 * Description:
 *
 *      Routine serializes/deserializes an CAPID structure.
 *
 * Return Values:
 *
 *      bool_t          - 1 successful xdr conversion
 *      bool_t          - 0 xdr conversion failed
 *
 * Implicit Inputs:
 *
 *      NONE
 *
 * Implicit Outputs:
 *
 *      NONE
 *
 * Considerations:
 *
 *      NONE
 *
 * Module Test Plan:
 *
 *      NONE
 *
 * Revision History:
 *
 *      H.I. Grapek     20-Aug-1991     Added support for NEW cap_id.
 */


/*
 *      Header Files:
 */
#include "csi.h"
#include "cl_pub.h"
#include "ml_pub.h"
 
 
 
/*
 *      Defines, Typedefs and Structure Definitions:
 */

/*
 *      Global and Static Variable Declarations:
 */
static char     *st_src = __FILE__;
static char     *st_module = "csi_xcap_id()";
 
 
/*
 *      Procedure Type Declarations:
 */



bool_t 
csi_xcap_id (
    XDR *xdrsp,                         /* xdr handle structure */
    CAPID *capidp                        /* cap_id */
)
{


#ifdef DEBUG
    if TRACE(CSI_XDR_TRACE_LEVEL)
        cl_trace(st_module,                     /* routine name */
                 2,                             /* parameter count */
                 (unsigned long) xdrsp,         /* argument list */
                 (unsigned long) capidp);       /* argument list */
#endif /* DEBUG */

    /* translate the acs */
    if (!csi_xacs(xdrsp, &capidp->lsm_id.acs)) {
        MLOGCSI((STATUS_TRANSLATION_FAILURE, st_module,  "csi_xacs()",
	  MMSG(928, "XDR message translation failure")));
        return(0);
    }

    /* translate the lsm */
    if (!csi_xlsm(xdrsp, &capidp->lsm_id.lsm)) {
        MLOGCSI((STATUS_TRANSLATION_FAILURE, st_module,  "csi_xlsm()", 
	  MMSG(928, "XDR message translation failure")));
        return(0);
    }

    /* translate the cap */
    if (!csi_xcap(xdrsp, &capidp->cap)) {
        MLOGCSI((STATUS_TRANSLATION_FAILURE, st_module,  "csi_xcap()", 
	  MMSG(928, "XDR message translation failure")));
        return(0);
    }

    return(1);

}
