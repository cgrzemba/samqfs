
#ifndef lint
static char SccsId[] = "@(#)csi_xstatus.c	5.5 3/6/94 ";
#endif
/*
 * Copyright (1989, 2011) Oracle and/or its affiliates.  All rights reserved.
 *
 * Name:
 *
 *      csi_xstatus()
 *
 * Description:
 *
 *      Routine serializes/deserializes a data of type STATUS.
 *
 * Return Values:
 *
 *      bool_t          - 1 successful xdr conversion
 *      bool_t          - 0 xdr conversion failed
 *
 * Implicit Inputs:
 *
 *      NONE
 *
 * Implicit Outputs:
 *
 *      NONE
 *
 * Considerations:
 *
 *      NONE
 *
 * Module Test Plan:
 *
 *      NONE
 *
 */


/*
 *      Header Files:
 */
#include "csi.h"
#include "csi_xdr_pri.h"
#include "ml_pub.h"
#include "cl_pub.h"
 
 
 
/*
 *      Defines, Typedefs and Structure Definitions:
 */

/*
 *      Global and Static Variable Declarations:
 */
static char     *st_module = "csi_xstatus()";
 
 
/*
 *      Procedure Type Declarations:
 */



bool_t 
csi_xstatus (
    XDR *xdrsp,                         /* xdr handle structure */
    STATUS *status                        /* status */
)
{


#ifdef DEBUG
    if TRACE(CSI_XDR_TRACE_LEVEL)
        cl_trace(st_module,                     /* routine name */
                 2,                             /* parameter count */
                 (unsigned long) xdrsp,         /* argument list */
                 (unsigned long) status);       /* argument list */
#endif /* DEBUG */

    /* translate status */
    if (!xdr_enum(xdrsp, (enum_t *)status)) {
        MLOGCSI((STATUS_TRANSLATION_FAILURE, st_module, "xdr_enum()", 
	  MMSG(928, "XDR message translation failure")));
        return(0);
    }

    return(1);

}
