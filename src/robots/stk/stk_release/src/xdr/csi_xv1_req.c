#ifndef lint
static char SccsId[] = "@(#)csi_xv1_req.c	5.6 1/19/94 ";
#endif
/*
 ...
 *
 * Description:
 *      Encode/Decode Version 1 requests.
 *
 * Return Values:
 ...
 *      E. A. Alongi        28-Aug-1992     deleted unnecessary test for
 *                                          EXTENDED bit.
 *      E. A. Alongi        02-Dec-1992     For the case COMMAND_SET_CAP,
 *                                          changed the incorrect function name
 *                                          parameter to csi_logevent() - from
 *                                          "xdr_u_short()" to "xdr_char()".
 *      E. A. Alongi        02-Jan-1994     Modifications from flint run.
...
static bool_t st_xau_request (XDR *xdrsp, CSI_V1_AUDIT_REQUEST *reqp);
static bool_t st_xqu_request (XDR *xdrsp, CSI_V1_QUERY_REQUEST *reqp);
static bool_t st_xva_request (XDR *xdrsp, CSI_V1_VARY_REQUEST *reqp);

/*
 * Global and Static Variable Declarations 
 */

static char     *st_module = "csi_xv1_req()";
...
csi_xv1_req (
    ...
    CSI_V1_REQUEST *reqp                           /* ptr into current request */
)
{
    register int    i;                          /* for counting identifiers */
    register int    count;                      /* holds pkt count for loop */
    register int    part_size = 0;              /* size on structure */
    register int    total_size = 0;             /* current total size */
    BOOLEAN         badsize;                    /* TRUE if pkt sizeing error */

    /*
     * handle request translation by command type 
     */
    switch (reqp->csi_req_header.message_header.command) {
        ...
        case COMMAND_CANCEL:
		  ...
		  MMSG(928, "XDR message translation failure")));
            ...
            csi_xcur_size = sizeof(CSI_V1_CANCEL_REQUEST);
            break;
        ...
        case COMMAND_DISMOUNT:
		  ...
		  MMSG(928, "XDR message translation failure")));
            ...
            /* re-calc size in case there is compiler padding on last element */
            csi_xcur_size = sizeof(CSI_V1_DISMOUNT_REQUEST);
            break;

        case COMMAND_EJECT:

            /* determine if volid format A or volume range format B */
            if (RANGE & reqp->csi_req_header.message_header.extended_options) {
                ...
                badsize   = CHECKSIZE(csi_xcur_size, part_size, csi_xexp_size);
                if (badsize || !csi_xv1_cap_id(xdrsp, 
        ...
        case COMMAND_ENTER:
            ...
            part_size = sizeof(V1_CAPID);
            badsize   = CHECKSIZE(csi_xcur_size, part_size, csi_xexp_size);
            if (badsize || !csi_xv1_cap_id(xdrsp,&reqp->csi_enter_req.cap_id)) {
                MLOGCSI((STATUS_TRANSLATION_FAILURE, st_module,
		  "csi_xv1_cap_id()", 
		  MMSG(928, "XDR message translation failure")));
                return 0;
            ...
            if (reqp->csi_venter_req.csi_request_header.message_header.
			  ...
			  MMSG(928, "XDR message translation failure")));
                ...
                csi_xcur_size = sizeof(CSI_V1_ENTER_REQUEST);
            }
            break;
        ...
        case COMMAND_IDLE:
            ...
            /* portability: size end of request header for compiler padding*/
            csi_xcur_size = sizeof(CSI_V1_IDLE_REQUEST);
        ...
        case COMMAND_START:
            ...
            /* portability: size end of request header for compiler padding*/
            csi_xcur_size = sizeof(CSI_V1_START_REQUEST);
        ...
        case COMMAND_SET_CAP:
		  ...
		  MMSG(928, "XDR message translation failure")));
            ...
            for (i=0, count=reqp->csi_set_cap_req.count; i < count; i++ ) {
                ...
                badsize   = CHECKSIZE(csi_xcur_size, part_size, csi_xexp_size);
                if (badsize || 
                    !csi_xv1_cap_id(xdrsp,&reqp->csi_set_cap_req.cap_id[i])){
                        MLOGCSI((STATUS_TRANSLATION_FAILURE, st_module,
			  "csi_xv1_cap_id()", 
			  MMSG(928, "XDR message translation failure")));
                    return 0;
        ...
        default:
            /* portability: size end of request header for compiler padding*/
            csi_xcur_size = sizeof(CSI_V1_REQUEST_HEADER);
 ...
 *
 * Description:
 *      Routine serializes/deserializes a partly translated CSI_V1_AUDIT_REQUEST
 *      structure from the position of the V1_CAPID, onward, downstream.  
 *
 ...
 *
 * Considerations:
 *      Translation must begin at V1_CAPID structure since this packet is 
...
st_xau_request (
    ...
    CSI_V1_AUDIT_REQUEST *reqp           /* partly translated audit request */
)
{
    ...
    badsize   = CHECKSIZE(csi_xcur_size, part_size, csi_xexp_size);
    if (badsize || !csi_xv1_cap_id(xdrsp, &reqp->cap_id)) {
 ...
 *
 * Description:
 *      Routine serializes/deserializes a CSI_V1_QUERY_REQUEST structure 
 *      from the point beginning with the type (type TYPE).  The
 *      data above that point in the CSI_V1_QUERY_REQUEST structure
...
st_xqu_request (
    XDR *xdrsp,            /* xdr handle structure */
    CSI_V1_QUERY_REQUEST *reqp             /* partly translated query request */
)
{
    register int        i;              /* loop counter */
    register int        count;          /* holds count value */
    ...
    for (i = 0, count = reqp->count; i < count; i++ ) {
        switch (reqp->type) {
         ...
         case TYPE_CAP:
                ...
                badsize   = CHECKSIZE(csi_xcur_size, part_size, csi_xexp_size);
                if (badsize || 
                    !csi_xv1_cap_id(xdrsp,&reqp->identifier.cap_id[i])){
                        MLOGCSI((STATUS_TRANSLATION_FAILURE, "st_xqu_request()",
			  "csi_xv1_cap_id()", 
			  MMSG(928, "XDR message translation failure")));
                    return(0);
 ...
 *
 * Description:
 *      Routine serializes/deserializes a CSI_V1_VARY_REQUEST structure 
 *      from the point beginning with the state (type STATE).  The
 *      data above that point in the CSI_V1_VARY_REQUEST structure
 *      should have already been translated.
 *
 *      The current packet size "csi_xcur_size" is increased by the size of the 
 *      valid portion of this part of this packet.
...
st_xva_request (
    XDR *xdrsp,        /* xdr handle structure */
    CSI_V1_VARY_REQUEST *reqp              /* vary request structure */
)
{
    register int        i;              /* loop counter */
    register int        count;          /* holds count value */
