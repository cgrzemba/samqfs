#ifndef lint
static char SccsId[] = "@(#)csi_xv2_res.c	5.5 11/11/93 ";
#endif
/*
 ...
 * Description:
 *
 *      Encode/Decode Version 2 responses.
 *
 * Return Values:
 *
 ...
 *                                              modified from csi_xv1_res().
 *	Emanuel A. Alongi   16-Aug-1993.    Changes to preserve translation of
 *					    version 2 response packets.
 *	Scott L. Siao       20-Mar-2002.    Added type cast to arguments of xdr 
 *                                          functions
...
static char     *st_module = "csi_xv2_res()";
 
/*
 * Procedure Type Declarations: 
 */
static bool_t st_xau_response (XDR *, CSI_V2_AUDIT_RESPONSE *);
static bool_t st_xva_response (XDR *, CSI_V2_VARY_RESPONSE *);
static bool_t st_xeject_enter (XDR *, CSI_V2_EJECT_ENTER *);


bool_t 
csi_xv2_res (
    XDR *xdrsp,                         /* xdr handle */
    CSI_V2_RESPONSE *resp               /* request pointer */
)
{
    ...
    case COMMAND_CANCEL:
        ...
        csi_xcur_size = sizeof(CSI_V2_CANCEL_RESPONSE);
        break;
    ...
    case COMMAND_DISMOUNT:
        ...
        /* re-size in case there is compiler tail padding on struture */
        csi_xcur_size = sizeof(CSI_V2_DISMOUNT_RESPONSE);
        break;
    ...
    case COMMAND_IDLE:
        ...
        /* portability: size end of response header for compiler padding*/
        csi_xcur_size = sizeof(CSI_V2_IDLE_RESPONSE);
    ...
    case COMMAND_MOUNT:
        ...
        /* re-size in case there is compiler tail padding on struture */
        csi_xcur_size = sizeof(CSI_V2_MOUNT_RESPONSE);
        break;
    ...
    case COMMAND_MOUNT_SCRATCH:
        ...
        /* re-size in case there is compiler tail padding on struture */
        csi_xcur_size = sizeof(CSI_V2_MOUNT_SCRATCH_RESPONSE);
        break;
    ...
    case COMMAND_QUERY:
        ...
        if (!csi_xv2quresponse(xdrsp, &resp->csi_query_res)) {
            MLOGCSI((STATUS_TRANSLATION_FAILURE, st_module,
	      "csi_xqu_response()", 
	      MMSG(928, "XDR message translation failure")));
            return 0;
    ...
    case COMMAND_START:
        ...
        /* portability: size end of response header for compiler padding*/
        csi_xcur_size = sizeof(CSI_V2_START_RESPONSE);
        /* nothing more to do */
        break;
 ...
 * 	CSI_V2_AUDIT_RESPONSE structure from the positiion of the CAPID, onward,
 *      downstream.
 *
...
st_xau_response (
    ...
    CSI_V2_AUDIT_RESPONSE *resp  /* start of partly translated audit response */
)
{
 ...
 *
 * Description:
 *      Routine serializes/deserializes a CSI_V2_VARY_RESPONSE structure 
 *      from the point beginning with the state (type STATE).  The
 *      data above that point in the CSI_V2_VARY_RESPONSE structure
...
st_xva_response (
    XDR *xdrsp,        /* xdr handle structure */
    CSI_V2_VARY_RESPONSE *resp        /* vary response structure */
)
{
 ...
 *
 * Description:
 *      Routine serializes/deserializes an CSI_V2_EJECT_ENTER structure/packet
...
st_xeject_enter (
    XDR *xdrsp,         /* xdr handle structure */
    CSI_V2_EJECT_ENTER *resp          /* eject enter structure */
)
{
    register int i;             /* loop counter */
    register int count;         /* holds count value */
