...
static int _getSubcommandProps(char *, subCommandProps_t **);
...
usage(uint_t usageType)
{
	...
	printf("%s:\t%s ", gettext("Usage"), commandName);

	for (i = 0; standardCmdOptions[i].name; i++) {
		printf("-%c", standardCmdOptions[i].val);
	...
	if (usageType == GENERAL_USAGE) {
		for (i = 0; standardSubCmdOptions[i].name; i++) {
			printf(",--%s", standardSubCmdOptions[i].name);
			if (standardSubCmdOptions[i+1].name)
				printf(",");
		}
	}
