/*
 *  lockfs.c - Lock a SAM-FS file system.
 *
 *	lockfs locks operations on a SAM-FS file system.
 *	lockfs unlocks operations on a SAM-FS file system.
 *
 */
...
getmntnames()
{
	...
	while ((getmntent(mnttab, mntp)) == 0) {
		if ((strcmp(mntp->mnt_fstype, MNTTYPE_UFS) != 0) &&
		    (strcmp(mntp->mnt_fstype, "samfs") != 0)) {
			continue;
		}
