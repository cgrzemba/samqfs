...
#ifndef _SYS_FS_QFS_LOG_H
#define	_SYS_FS_QFS_LOG_H

#ifdef sun
#pragma ident	"$Revision: 1.7 $"

/*
 * The QFS Journaling (LQFS) infrastructure was ported from the
 * Solaris 10 version of the UFS Journaling (LUFS) infrastructure.
 * Time-to-market with reasonable stability was the major factor
 * in this decision.  Initially supported only for Sun's QFS NAS
 * product, with intent to migrate to general SAM-QFS later.
 * Some technical background and details of the LUFS infrastructure
 * can be found at:
 *
 * http://fs.central/projects/lufs/docs/white_paper/white_paper.html
 * http://fs.central/projects/lufs/perf_proj2/summary.txt
 *
 * Test descriptions can be found at:
 *
 * http://fs.central/testing/tsufs/tsufs_test_desc.txt
 *
 * The tests LGTS, LLFTS, lufs, TestLUFS, ShootLUFS, and TransLUFS
 * focus on LUFS, but most tests run once with logging and once
 * without.
 */
...
typedef struct ic_extent {
	...
	int		ic_ord;		/* Extent ordinal */
} ic_extent_t;

typedef struct extent_block {
	uint32_t	type;		/* Set to LQFS_EXTENTS to identify */
					/*   structure on disk. */
	...
	int		nextord;	/* ordinal of next extent_block. */
	extent_t	extents[1];
} extent_block_t;
...
typedef struct ic_extent_block {
	...
	int		ic_nextord;	/* ordinal of next extent_block. */
 ...
 * from the file system size according to the calculation in lqfs_enable()
 */
...
#define	LQFS_VERSION		(UINT32_C(1))	/* Version 1 */
#define	LQFS_VERSION_LATEST	LQFS_VERSION
...
typedef struct ml_odunit {
	 ...
	 * Misc
	 */
	timeval_lqfs_common_t	od_timestamp;	/* time of last state change */
} ml_odunit_t;
...
typedef struct ml_unit {
	...
	qfsvfs_t	*un_qfsvfs;	/* backpointer to qfsvfs */
...
typedef struct crb {
	...
	uchar_t		c_ord;		/* master file ordinal of buffer */
...
struct delta {
	int64_t		d_mof;	/* byte offset on device to start writing */
				/*   delta */
	uchar_t		d_ord;	/* ordinal of device to start writing delta */
	int32_t		d_nb;	/* # bytes in the delta */
	delta_t 	d_typ;	/* Type of delta.  Defined in qfs_trans.h */
};
/*
 * common map entry
 */
...
/*
 * MAP definitions.  nhash count is per QFS file system ordinal.
 */
#define	DELTAMAP_NHASH	(512)
#define	LOGMAP_NHASH	(2048)
#define	MAP_INDEX(mof, mtm) \
	(((mof) >> MAPBLOCKSHIFT) & (mtm->mtm_nhash-1))
#define	MAP_ORD_OFFSET(ord, mtm) ((ord) * (mtm)->mtm_nhash)
#define	MAP_HASH(mof, ord, mtm) \
	((mtm)->mtm_hash + (MAP_ORD_OFFSET((ord), (mtm))) + \
		(MAP_INDEX((mof), (mtm))))
...
typedef struct mt_map {
	...
	ulong_t		mtm_nhash;	/* # of hash anchors per QFS ordinal */
	uchar_t		mtm_maxord;	/* max ordinal of QFS file system */
	...
	/*
	 * logmap only condition variable, to synchronize with lqfs_unsnarf.
	 */
	kcondvar_t	mtm_cv;
} mt_map_t;
 ...
 */
#define	LQFS_OVERLAP(sof, sord, snb, dof, dord, dnb) \
	(((sord) == (dord)) && \
	(((sof) >= (dof) && (sof) < ((dof) + (dnb))) || \
	((dof) >= (sof) && (dof) < ((sof) + (snb)))))
#define	WITHIN(sof, sord, snb, dof, dord, dnb) \
	(((sord) == (dord)) && \
	(((sof) >= (dof)) && (((sof) + (snb)) <= ((dof) + (dnb)))))
#define	DATAoverlapME(mof, ord, hnb, me) \
	(LQFS_OVERLAP((mof), (ord), (hnb), (me)->me_mof, (me)->me_ord, \
		(me)->me_nb))
#define	MEwithinDATA(me, mof, ord, hnb) \
	(WITHIN((me)->me_mof, (me)->me_ord, (me)->me_nb, (mof), (ord), (hnb)))
#define	DATAwithinME(mof, ord, hnb, me) \
	(WITHIN((mof), (ord), (hnb), (me)->me_mof, (me)->me_ord, (me)->me_nb))
#define	DATAwithinCRB(mof, ord, nb, crb) \
	(WITHIN((mof), (ord), (nb), (crb)->c_mof, (crb)->c_ord, (crb)->c_nb))
...
/*
 * fio_lqfs_stats_t is used by _FIO_GET_TOP_STATS ioctl for getting topstats
 */
typedef struct fio_lqfs_stats {
 ...
 *	MT_TRACE_COMMON		- trace transactions (used with MT_TRANSACT)
...
#define	MT_TRACE_COMMON		(0x00000020)
...
lqfs_find_ord(qfsvfs_t *qfsvfsp, buf_t *bp)
{
	uchar_t ord;

	for (ord = 0; ord < qfsvfsp->mt.fs_count; ord++) {
		if (qfsvfsp->mi.m_fs[ord].dev == bp->b_edev) {
			break;
		}
	}
	ASSERT(ord != qfsvfsp->mt.fs_count);

	return (ord);
}

/*
 * Log layer protos -- lqfs_log.c
 */
...
extern void		ldl_write(ml_unit_t *, caddr_t, offset_t, uchar_t,
				mapentry_t *);
extern void		ldl_waito(ml_unit_t *);
extern int		ldl_read(ml_unit_t *, caddr_t, offset_t, uchar_t,
				off_t, mapentry_t *);
...
extern void		lqfs_strategy(ml_unit_t *, buf_t *);
extern void		lqfs_read_strategy(ml_unit_t *, buf_t *);
extern void		lqfs_write_strategy(ml_unit_t *, buf_t *);
extern void		lqfs_init(void);
extern int		lqfs_enable(struct vnode *, struct fiolog *, cred_t *);
extern int		lqfs_disable(vnode_t *, struct fiolog *);

/*
 * transaction op layer -- lqfs_top.c
 */
...
/*
 * map layer -- lqfs_map.c
 */
extern void		map_free_entries(mt_map_t *);
extern int		matamap_overlap(mt_map_t *, offset_t, uchar_t, off_t);
extern int		matamap_within(mt_map_t *, offset_t, uchar_t, off_t);
extern int		deltamap_need_commit(mt_map_t *);
extern void		deltamap_add(mt_map_t *, offset_t, uchar_t, off_t,
				delta_t, int (*)(), uint64_t,
				threadtrans_t *tp);
extern mapentry_t	*deltamap_remove(mt_map_t *, offset_t, uchar_t, off_t);
extern void		deltamap_del(mt_map_t *, offset_t, uchar_t, off_t);
...
extern int		logmap_overlap(mt_map_t *, offset_t, uchar_t, off_t);
extern void		logmap_remove_roll(mt_map_t *, offset_t, uchar_t,
				off_t);
extern int		logmap_next_roll(mt_map_t *, offset_t *, uchar_t *);
extern int		logmap_list_get(mt_map_t *, offset_t, uchar_t, off_t,
				mapentry_t **);
extern int		logmap_list_get_roll(mt_map_t *, offset_t, uchar_t,
				rollbuf_t *);
...
extern void		logmap_make_space(struct mt_map *, ml_unit_t *,
				mapentry_t *);
extern void		logmap_add(ml_unit_t *, char *, offset_t, uchar_t,
				mapentry_t *);
extern void		logmap_add_buf(ml_unit_t *, char *, offset_t, uchar_t,
...
extern void		logmap_cancel(ml_unit_t *, offset_t, uchar_t, off_t,
				int);
extern void		logmap_free_cancel(mt_map_t *, mapentry_t **);
extern int		logmap_iscancel(mt_map_t *, offset_t, uchar_t, off_t);
...
/*
 * scan and roll threads -- lqfs_thread.c
 */
...
extern int	top_write_debug(ml_unit_t *, mapentry_t *, offset_t, uchar_t,
			off_t);
extern int	matamap_overlap(mt_map_t *, offset_t, uchar_t, off_t);
...
extern int	map_check_ldl_write(ml_unit_t *, caddr_t, offset_t, uchar_t,
			mapentry_t *);
extern int	logmap_logscan_commit_debug(off_t, mt_map_t *);
extern int	logmap_logscan_add_debug(struct delta *, mt_map_t *);
extern int	top_delta_debug(ml_unit_t *, offset_t, uchar_t, off_t, delta_t);
...
extern int	lqfs_initialize_debug(ml_odunit_t *);
#endif	/* DEBUG */

extern int lqfs_flush(struct sam_mount *);

extern uint64_t delta_stats[DT_MAX];
extern uint64_t roll_stats[DT_MAX];
extern struct logstats logstats;
extern int qfs_crb_enable;

extern uint_t topkey;
extern uint32_t qfs_ncg_log;
...
#else
#define	TRANS_INODE(mp, ip)	while (0) { }
#define	TRANS_BEGIN_CSYNC(mp, issync, tid, size)	while (0) { }
#define	TRANS_END_CSYNC(mp, error, issync, tid, size)	while (0) { }
#define	TRANS_ISTRANS(mp)	(0)
#endif /* sun */

#endif	/* _SYS_FS_QFS_LOG_H */
