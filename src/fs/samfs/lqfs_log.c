...
extern struct kmem_cache	*lqfs_sv;
extern struct kmem_cache	*lqfs_bp;

extern void ml_unit_validate(ml_unit_t *ul);
...
makebusy(ml_unit_t *ul, buf_t *bp)
{
	...
	if (bp->b_flags & B_READ) {
		ldl_seterror(ul, "Error reading qfs log");
	} else {
		ldl_seterror(ul, "Error writing qfs log");
	}
}
...
ldl_strategy(ml_unit_t *ul, buf_t *pb)
{
	...
	qfsvfsp = ul->un_qfsvfs;
	dev	= ul->un_dev;
	read = pb->b_flags & B_READ;

	/*
	 * Allocate and initialise the save stucture,
	 */
	sv = kmem_cache_alloc(lqfs_sv, KM_SLEEP);
	sv->sv_error = 0;
	sv->sv_bp = pb;
	nb_left = pb->b_bcount;
	...
	do {
		...
		lbp = kmem_cache_alloc(lqfs_bp, KM_SLEEP);
		...
		if (error) {
			...
			if (qfsvfsp->vfs_snapshot) {
				(void) tsd_set(bypass_snapshot_throttle_key, \
				    (void *)1);
				fssnap_strategy(&qfsvfsp->vfs_snapshot, cb);
				...
				if ((cb->b_flags & B_READ) == 0) {
					LQFS_MSG(CE_WARN, "ldl_strategy(): "
					    "bdev_strategy writing "
					...
					LQFS_MSG(CE_WARN, "ldl_strategy(): "
					    "bdev_strategy reading "
					    "mof 0x%x edev %ld nb %d\n",
					    cb->b_blkno * 512,
					    cb->b_edev,
					    cb->b_bcount);
				}
				error = bdev_strategy(cb);
#ifdef LQFS_TODO_SNAPSHOT
			}
#endif /* LQFS_TODO_SNAPSHOT */
		}
...
again:
	...
	if ((nb_in_sec -= copy_nb) == 0) {
		st = (sect_trailer_t *)(void *)(bp->b_un.b_addr + bp->b_bcount);
...
static void
fetchzeroes(caddr_t dst_va, offset_t dst_mof, uchar_t dst_ord, ulong_t dst_nb,
		mapentry_t *me)
{
	offset_t	src_mof	= me->me_mof;
	size_t		src_nb	= me->me_nb;
	uchar_t		src_ord	= me->me_ord;

	ASSERT(src_ord == dst_ord);
...
/* ARGSUSED3 */
void
ldl_write(ml_unit_t *ul, caddr_t bufp, offset_t bufmof, uchar_t buford,
		struct mapentry *me)
{
	buf_t		*bp;
	caddr_t		va;
...
ldl_savestate(ml_unit_t *ul)
{
	...
	if (error) {
		ldl_seterror(ul, "Error writing qfs log state");
	}
}
...
ldl_logscan_read(ml_unit_t *ul, off_t *lofp, size_t nb, caddr_t va)
{
	 ...
	 */
	if (ul->un_head_lof < ul->un_tail_lof) {
		if (!WITHIN(*lofp, 0, nb, ul->un_head_lof, 0,
		    (ul->un_tail_lof - ul->un_head_lof))) {
			return (EIO);
		}
	} else {
		if (LQFS_OVERLAP(*lofp, 0, nb, ul->un_tail_lof, 0,
		    (ul->un_head_lof - ul->un_tail_lof))) {
			return (EIO);
		}
	}
...
ldl_need_roll(ml_unit_t *ul)
{
	 ...
	 */
	result = (busybytes > (nb - (nb >> 2)));
/* TEST - Might want to set result lower for more frequent rolls. */
	return (result);
}
...
ldl_seterror(ml_unit_t *ul, char *why)
{
	...
	cmn_err(CE_WARN, "qfs log for %s changed state to Error",
	    ul->un_qfsvfs->vfs_fs->fs_fsmnt);
	cmn_err(CE_WARN, "Please umount(1M) %s and run samfsck(1M)",
	    ul->un_qfsvfs->vfs_fs->fs_fsmnt);

	/*
	 * If we aren't in the middle of scan (aka snarf); tell qfs
	 * to hard lock itself.
	 */
