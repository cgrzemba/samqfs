...
static void
top_trace(enum delta_type dtyp, dev_t dev, long long arg1, long arg2,
    long arg3)
{
	if (toptrace == NULL) {
		toptraceindex = 0;
 ...
 */
void
top_mataadd(qfsvfs_t *qfsvfsp, offset_t mof, uchar_t ord, off_t nb)
{
...
#endif /* LUFS */
	deltamap_add(ul->un_matamap, mof, ord, nb, 0, 0, 0, NULL);
}
 ...
 */
void
top_matadel(qfsvfs_t *qfsvfsp, offset_t mof, uchar_t ord, off_t nb)
{
	ml_unit_t	*ul	= LQFS_GET_LOGP(qfsvfsp);
	...
	ASSERT((VFS_PTR(qfsvfsp))->vfs_dev == ul->un_dev);
#endif /* LUFS */
	ASSERT(!matamap_overlap(ul->un_deltamap, mof, ord, nb));
	deltamap_del(ul->un_matamap, mof, ord, nb);
}
...
top_begin_debug(ml_unit_t *ul, top_t topid, ulong_t size)
{
	...
	if (ul->un_debug & MT_TRACE_COMMON) {
...
int
top_delta_debug(ml_unit_t *ul, offset_t mof, uchar_t ord, off_t nb,
    delta_t dtyp)
{
	...
	if (ul->un_debug & MT_TRANSACT) {
		...
		case DT_CANCEL:
		case DT_ABZERO:
			if (!matamap_within(ul->un_deltamap, mof, ord, nb)) {
	...
	if (ul->un_debug & MT_TRACE_COMMON) {
		top_trace(dtyp, ul->un_dev, mof, ord, (long)nb);
	}
 ...
 */
int
map_check_ldl_write(ml_unit_t *ul, caddr_t va, offset_t vamof, uchar_t vaord,
			mapentry_t *me)
{
	caddr_t		bufp;
	...
	/* Do the read */
	me->me_agenext = NULL;
	if (ldl_read(ul, bufp, me->me_mof, me->me_ord, me->me_nb, me) == 0) {
		ASSERT(vaord == me->me_ord);
...
map_check_linkage(mt_map_t *mtm)
{
	...
	off_t		firstlof;
	int		wrapped;
	qfsvfs_t	*qfsvfsp = mtm->mtm_ul->un_qfsvfs;
	...
	for (i = 0; i < mtm->mtm_nhash * qfsvfsp->mt.fs_count; ++i) {
		for (me = *mtm->mtm_hash; me; me = me->me_hash) {
			++hashed;
	...
	preved = 0;
	for (me = mtm->mtm_prev; me != (mapentry_t *)(void *)mtm;
	    me = me->me_prev) {
		preved++;
	}
	...
	if (mtm->mtm_type == logmaptype) {
		...
		for (me = mtm->mtm_next->me_next;
			...
			if (firstlof == 0) {
				olof = me->me_lof;
				firstlof = olof;
				if (me->me_next != (mapentry_t *)(void *)mtm) {
					me = me->me_next;
				}
				continue;
			}
 ...
 */
int
matamap_overlap(mt_map_t *mtm, offset_t mof, uchar_t ord, off_t nb)
{
	off_t		hnb;
	mapentry_t	*me;
	mapentry_t	**mep;
 ...
 */
int
matamap_within(mt_map_t *mtm, offset_t mof, uchar_t ord, off_t nb)
{
	off_t		hnb;
	mapentry_t	*me;
	mapentry_t	**mep;
	...
	for (hnb = 0; nb && scans == withins; nb -= hnb, mof += hnb) {
		...
		mutex_enter(&mtm->mtm_mutex);
		for (me = *mep; me; me = me->me_hash) {
			if ((DATAwithinME(mof, ord, hnb, me)) != 0) {
				withins++;
				break;
			}
		}
...
int
lqfs_initialize_debug(ml_odunit_t *ud)
{
	ud->od_debug = lqfs_debug;
	return (1);
}
...
/*
 * lqfs_debug controls the debug level for TSqfs, and is only used
 * for a debug kernel. It's referenced by qfs_ioctl() and so is
