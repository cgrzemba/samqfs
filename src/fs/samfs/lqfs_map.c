...
extern struct kmem_cache *lqfs_bp;
#ifdef LQFS_TODO_QUOTAS
extern int qfs_trans_push_quota();
#endif /* LQFS_TODO_QUOTAS */

/*
 * globals
 */
kmem_cache_t *mapentry_cache = NULL;
...
uint64_t qfs_crb_size = 0;		/* current size of all crb buffers */
uint64_t qfs_crb_max_size = 0;		/* highest crb buffer use so far */
size_t qfs_crb_limit;			/* max allowable size for crbs */
uint64_t qfs_crb_alloc_fails = 0;	/* crb allocation failures stat */
#define	QFS_MAX_CRB_DEFAULT_DIVISOR 10	/* max 1/10 kmem_maxavail() */
int qfs_max_crb_divisor = QFS_MAX_CRB_DEFAULT_DIVISOR; /* tunable */
...
#define	CRB_FREE(crb, me) \
	...
	atomic_add_64(&qfs_crb_size, -(uint64_t)crb->c_nb); \
 ...
 * qfs_trans_push_quota(), then check that the old and new deltas differ.
 * If so we clean up with handle_dquot() before replacing the old delta.
 */
#define	HANDLE_DQUOT(me, melist) { \
	if ((me->me_arg) && \
	    (me->me_func == qfs_trans_push_quota)) { \
...
map_put(mt_map_t *mtm)
{
	...
	kmem_free(mtm->mtm_hash,
	    (size_t) (sizeof (mapentry_t *) * mtm->mtm_nhash * L_FSET));
...
map_get(ml_unit_t *ul, enum maptypes maptype, int nh)
{
	 ...
	 * Hash space is allocated using maximum ordinal to cleanly support
	 * online-grow.
	 */
	...
	mtm->mtm_next = (mapentry_t *)(void *)mtm;
	mtm->mtm_prev = (mapentry_t *)(void *)mtm;
	...
	threadtrans_t *tp)
{
	...
	for (hnb = 0; nb; nb -= hnb, mof += hnb) {
		 ...
		 * reference the dquot structure which will not be
		 ...
		 */
		LQFS_MSG(CE_WARN, "deltamap_add(): Checking map hash "
		    "chain %d for entry containing mof 0x%x ord %d nb %d\n",
		    (MAP_ORD_OFFSET(ord, mtm) + MAP_INDEX(mof, mtm)),
		    mof, ord, hnb);
		mep = MAP_HASH(mof, ord, mtm);
		for (me = *mep; me; me = me->me_hash) {
			if (DATAwithinME(mof, ord, hnb, me)) {
#ifdef LQFS_TODO_QUOTAS
				if (me->me_func == qfs_trans_push_quota) {
					/*
					 * Don't remove quota entries which have
					 * incremented the ref count (those
					 * with a qfs_trans_push_quota push
					 * function).  Let logmap_add[_buf]
					 * clean them up.
					 */
					continue;
				}
			...
			ASSERT((dtyp == DT_CANCEL) ||
			    (!DATAoverlapME(mof, ord, hnb, me)) ||
			    MEwithinDATA(me, mof, ord, hnb));
		}
		 ...
		 * before the deltamap_push by lqfs_write_strategy
		 * and so multiple deltas to the same mof offset and ordinal
		...
		me->me_hash = *mep;
		*mep = me;
		me->me_next = (mapentry_t *)(void *)mtm;
...
/*
 * remove deltas within (mof, ord, nb) and return as linked list
 */
mapentry_t *
deltamap_remove(mt_map_t *mtm, offset_t mof, uchar_t ord, off_t nb)
{
	off_t		hnb;
	mapentry_t	*me;
	mapentry_t	**mep;
	...
	for (mer = NULL, hnb = 0; nb; nb -= hnb, mof += hnb) {
		 ...
		 */
		LQFS_MSG(CE_WARN, "deltamap_remove(): Removing map hash "
		    "chain %d entries within mof 0x%x ord %d nb %d\n",
		    (MAP_ORD_OFFSET(ord, mtm) + MAP_INDEX(mof, mtm)),
		    mof, ord, hnb);
		mep = MAP_HASH(mof, ord, mtm);
		while ((me = *mep) != 0) {
			if (MEwithinDATA(me, mof, ord, hnb)) {
				*mep = me->me_hash;
				me->me_next->me_prev = me->me_prev;
				me->me_prev->me_next = me->me_next;
				me->me_hash = mer;
...
/*
 * delete entries within (mof, ord, nb)
 */
void
deltamap_del(mt_map_t *mtm, offset_t mof, uchar_t ord, off_t nb)
{
	mapentry_t	*me;
	mapentry_t	*menext;

	menext = deltamap_remove(mtm, mof, ord, nb);
	while ((me = menext) != 0) {
		menext = me->me_hash;
		kmem_cache_free(mapentry_cache, me);
	}
...
deltamap_push(ml_unit_t *ul)
{
	...
	while ((me = mtm->mtm_next) != (mapentry_t *)(void *)mtm) {
		...
		if ((ul->un_flags & LDL_ERROR) ||
		    (*func)(ul->un_qfsvfs, dtyp, arg)) {
			deltamap_del(mtm, mof, ord, nb);
		}
	}
...
logmap_start_roll(ml_unit_t *ul)
{
	...
	if ((logmap->mtm_flags & MTM_ROLL_RUNNING) == 0) {
		...
		(void) thread_create(NULL, 0, trans_roll, (caddr_t)ul, 0, &p0,
		    TS_RUN, minclsyspri);
	}
...
/*
 * remove rolled deltas within (mof, ord, nb) and free them
 */
void
logmap_remove_roll(mt_map_t *mtm, offset_t mof, uchar_t ord, off_t nb)
{
	int		dolock = 0;
	off_t		hnb;
	mapentry_t	*me;
...
again:
	...
	for (hnb = 0; nb; nb -= hnb, mof += hnb) {
		 ...
		 * NOTE: QFS caller already adjusted mof to be on
		 * block boundary (RJS).
		 */
		LQFS_MSG(CE_WARN, "logmap_remove_roll(): Removing rolled map "
		    "hash chain %d entries within mof 0x%x ord %d nb %d\n",
		    (MAP_ORD_OFFSET(ord, mtm) + MAP_INDEX(mof, mtm)),
		    mof, ord, hnb);
		mep = MAP_HASH(mof, ord, mtm);
		while ((me = *mep) != 0) {
			if ((me->me_flags & ME_ROLL) &&
				...
				*mep = me->me_hash;
				...
				mtm->mtm_nme--;
				LQFS_MSG(CE_WARN, "logmap_remove_roll(): "
				    "Removed mof 0x%x ord %d nb %d (nme %d)\n",
				    me->me_mof, me->me_ord, me->me_nb,
				    mtm->mtm_nme);
				/*
 ...
 *	      to the master file disk offset and *ordp points
 *	      to the master file disk ordinal
 */
int
logmap_next_roll(mt_map_t *logmap, offset_t *mofp, uchar_t *ordp)
{
	mapentry_t *me;
	...
	me = logmap->mtm_next;
	for (me = logmap->mtm_next; me != (mapentry_t *)(void *)logmap;
	    me = me->me_next) {
		/* already rolled */
		if (me->me_flags & ME_ROLL) {
			continue;
...
/*
 * get a list of deltas within <mof, mof+nb> on ordinal <ord>
...
again:
	...
	*age = NULL;
	...
	for (hnb = 0; nb; nb -= hnb, mof += hnb) {
		 ...
		 */
		LQFS_MSG(CE_WARN, "logmap_list_get(): Finding map hash "
		    "chain %d entries that contain or are within mof 0x%x "
		    "ord %d hnb %d\n", (MAP_ORD_OFFSET(ord, mtm) +
		    MAP_INDEX(mof, mtm)), mof, ord, hnb);
		mep = MAP_HASH(mof, ord, mtm);
		for (me = *mep; me; me = me->me_hash) {
			if (me->me_dt == DT_CANCEL) {
				continue;
			}
			if (!DATAoverlapME(mof, ord, hnb, me)) {
				continue;
			}
			/*
			...
			if (me->me_flags & ME_AGE) {
				...
				crb = me->me_crb;
				if (crb) {
					if (DATAwithinCRB(savmof, savord, savnb,
					    crb)) {
						entire = 1;
					}
				} else {
					if (DATAwithinME(savmof, savord, savnb,
					    me)) {
						entire = 1;
					}
				}
 ...
 */
int
logmap_list_get_roll(mt_map_t *logmap, offset_t mof, uchar_t ord,
			rollbuf_t *rbp)
{
	 ...
	 */
	mutex_enter(&logmap->mtm_mutex);
	LQFS_MSG(CE_WARN, "logmap_list_get_roll(): Checking map hash "
	    "chain %d for entries within mof 0x%x ord %d nb 8K\n",
	    (MAP_ORD_OFFSET(ord, logmap) + MAP_INDEX(mof, logmap)), mof, ord);
	mep = MAP_HASH(mof, ord, logmap);
	for (me = *mep; me; me = me->me_hash) {
		if (!DATAoverlapME(mof, ord, MAPBLOCKSIZE, me)) {
			continue;
		}
		if (me->me_tid == logmap->mtm_tid) {
			continue;
		...
		/*
		 * Check if map entry is in use (by lqfs_read_strategy())
		...
		if (me->me_flags & ME_AGE) {
			...
			/* add mapentry to age ordered list */
			LQFS_MSG(CE_WARN, "logmap_list_get_roll(): Adding to "
			    "roll list mof 0x%x ord %d nb %d\n",
			    me->me_mof, me->me_ord, me->me_nb);
			logmap_list_age(&age, me);
		}
	}
...
out:
	...
	if (rbp->rb_crb != NULL) {
		LQFS_MSG(CE_WARN, "logmap_list_get_roll(): Returning CRB "
		    "c_mof 0x%x c_ord %d c_nb %d\n", rbp->rb_crb->c_mof,
		    rbp->rb_crb->c_ord, rbp->rb_crb->c_nb);
	} else {
		LQFS_MSG(CE_WARN, "logmap_list_get_roll(): Not returning "
		    "CRB\n");
	}
...
#define	QFS_RW_BALANCE 2
int qfs_rw_balance = QFS_RW_BALANCE;
...
logmap_setup_read(mapentry_t *age, rollbuf_t *rbp)
{
	...
	if (all_inodes) {
		 ...
		 * An unpublished tunable is defined (qfs_rw_balance) that
	...
	LQFS_MSG(CE_WARN, "logmap_setup_read(): Returning sector map 0x%x "
	    "read_needed %d for mof 0x%x (rbp->rb_bh.b_blkno mof 0x%x, "
	    "nb 8K)\n", secmap, read_needed, mof, rbp->rb_bh.b_blkno*512);
	return (read_needed);
}
...
logmap_abort(ml_unit_t *ul, uint32_t tid)
{
	 ...
	 * commit failed.
	 */
	for (i = 0; i < (mtm->mtm_nhash * qfsvfsp->mt.fs_count); i++) {
		mep = &mtm->mtm_hash[i];
		while ((me = *mep) != NULL) {
			if (me->me_tid == tid ||
...
logmap_add(
	...
	uchar_t vaord,			/* Ordinal of master for buf start */
	mapentry_t *melist)		/* Entries to add */
{
	...
	while (melist) {
		 ...
		 */
		LQFS_MSG(CE_WARN, "logmap_add(): Checking map hash chain %d "
		    "for entries within mof 0x%x ord %d nb %d\n",
		    (MAP_ORD_OFFSET(ord, mtm) + MAP_INDEX(mof, mtm)),
		    mof, ord, nb);
		savmep = mep = MAP_HASH(mof, ord, mtm);
		mutex_enter(&mtm->mtm_mutex);
		while ((me = *mep) != 0) {
			/*
			...
			if (MEwithinDATA(me, mof, ord, nb) &&
				...
				if (tid == me->me_tid &&
					...
					*mep = me->me_hash;
					...
					ASSERT(!(me->me_flags & ME_USER));
					mtm->mtm_nme--;
					LQFS_MSG(CE_WARN, "logmap_add(): "
					    "Removed mof 0x%x ord %d nb %d "
					    "(nme %d)\n", me->me_mof,
					    me->me_ord, me->me_nb,
					    mtm->mtm_nme);
#ifdef LQFS_TODO_QUOTAS
		 ...
		 */
		if (va) {
			ldl_write(ul, va, vamof, vaord, me);
		}
		if (ul->un_flags & LDL_ERROR) {
			kmem_cache_free(mapentry_cache, me);
			continue;
		...
		ASSERT((va == NULL) ||
		    ...
		    map_check_ldl_write(ul, va, vamof, vaord, me));
		...
		*savmep = me;
		...
		mtm->mtm_nme++;
		mtm->mtm_nmet++;
		LQFS_MSG(CE_WARN, "logmap_add(): Added mof 0x%x ord %d nb %d "
		    "(nme %d)\n", me->me_mof, me->me_ord, me->me_nb,
		    mtm->mtm_nme);
		mutex_exit(&mtm->mtm_mutex);
	}
...
logmap_add_buf(
	...
	uchar_t buford,			/* Ordinal of master for buf start */
	mapentry_t *melist,		/* Entries to add */
	...
	uint32_t bufsz)			/* Size of buf */
{
	...
	while (melist) {
		 ...
		 */
		LQFS_MSG(CE_WARN, "logmap_add_buf(): Checking map hash "
		    "chain %d for entries within mof 0x%x ord %d nb %d\n",
		    (MAP_ORD_OFFSET(ord, mtm) + MAP_INDEX(mof, mtm)),
		    mof, ord, nb);
		savmep = mep = MAP_HASH(mof, ord, mtm);
		mutex_enter(&mtm->mtm_mutex);
		while ((me = *mep) != 0) {
			...
			if ((me->me_dt == DT_INODE) && (tid == me->me_tid) &&
			    !crbsav && crb &&
			    WITHIN(mof, ord, nb, crb->c_mof, crb->c_ord,
			    crb->c_nb)) {
				ASSERT(crb->c_mof == bufmof);
				ASSERT(crb->c_ord == buford);
		...
		if ((crbsav == NULL) && (melist->me_dt != DT_ABZERO)) {
			 ...
			 */
			if (((qfs_crb_size + bufsz) < qfs_crb_limit) &&
			    (kmem_avail() > bufsz)) {
			...
			if (crbsav) {
				...
				if (crbsav->c_buf) {
					atomic_add_64(&qfs_crb_size,
					    (uint64_t)bufsz);
					if (qfs_crb_size > qfs_crb_max_size) {
						qfs_crb_max_size = qfs_crb_size;
					}
		...
		ASSERT(va);
		ldl_write(ul, va, bufmof, buford, me); /* add to on-disk log */
		if (ul->un_flags & LDL_ERROR) {
			CRB_RELE(me);
			kmem_cache_free(mapentry_cache, me);
			continue;
		...
		ASSERT(((mtm->mtm_debug & MT_LOG_WRITE_CHECK) == 0) ||
		    map_check_ldl_write(ul, va, bufmof, buford, me));
		...
		*savmep = me;
		...
		mtm->mtm_nmet++;
		if (me->me_crb) {
			LQFS_MSG(CE_WARN, "logmap_add_buf(): Added mof 0x%x "
			    "ord %d nb %d CRB mof 0x%x ord %d nb %d (nme %d)\n",
			    me->me_mof, me->me_ord, me->me_nb,
			    me->me_crb->c_mof, me->me_crb->c_ord,
			    me->me_crb->c_nb, mtm->mtm_nme);
		} else {
			LQFS_MSG(CE_WARN, "logmap_add_buf(): Added mof 0x%x "
			    "ord %d nb %d NO CRB (nme %d)\n", me->me_mof,
			    me->me_ord, me->me_nb, mtm->mtm_nme);
		}
		mutex_exit(&mtm->mtm_mutex);
	}
	...
	while ((me = *cancelhead) != NULL) {
		...
		*cancelhead = me->me_cancel;
		...
		if (me->me_flags & ME_HASH) {
			LQFS_MSG(CE_WARN, "logmap_free_cancel(): Freeing "
			    "cancelled delta in hash chain %d - entry "
			    "mof 0x%x ord %d\n",
			    (MAP_ORD_OFFSET(me->me_ord, mtm) +
			    MAP_INDEX(me->me_mof, mtm)),
			    me->me_mof, me->me_ord);
			mep = MAP_HASH(me->me_mof, me->me_ord, mtm);
			while (*mep) {
				if (*mep == me) {
					*mep = me->me_hash;
					me->me_next->me_prev = me->me_prev;
					...
					if (!(me->me_flags & ME_USER)) {
						...
						LQFS_MSG(CE_WARN,
						    "logmap_free_cancel(): "
						    "Removed mof 0x%x ord %d "
						    "nb %d (nme %d)\n",
						    me->me_mof, me->me_ord,
						    me->me_nb, mtm->mtm_nme);
					}
					break;
				} else {
					mep = &(*mep)->me_hash;
...
logmap_commit(ml_unit_t *ul, uint32_t tid)
{
	...
	if (mtm->mtm_dirty) {
		...
		ldl_write(ul, NULL, (offset_t)0, 0, &me);
		ldl_round_commit(ul);
...
logmap_sethead(mt_map_t *mtm, ml_unit_t *ul)
{
	...
	mutex_enter(&mtm->mtm_mutex);
	me = mtm->mtm_next;
	while (me != (mapentry_t *)(void *)mtm && me->me_lof == 0) {
		me = me->me_next;
	}

	if (me == (mapentry_t *)(void *)mtm) {
		lof = -1;
	} else {
		lof = me->me_lof;
		tid = me->me_tid;
...
logmap_settail(mt_map_t *mtm, ml_unit_t *ul)
{
	...
	mutex_enter(&ul->un_log_mutex);
	mutex_enter(&mtm->mtm_mutex);
	if (mtm->mtm_prev == (mapentry_t *)(void *)mtm) {
		lof = -1;
	} else {
		/*
...
again:
	 ...
	 */
	mutex_enter(&mtm->mtm_mutex);
	for (me = mtm->mtm_next; me != (mapentry_t *)(void *)mtm;
	    me = me->me_next) {
		if (me->me_flags & ME_ROLL) {
			break;
		}
	...
	mutex_exit(&mtm->mtm_mutex);
#ifdef LUFS
	(void) qfs_putsummaryinfo(ul->un_dev, qfsvfsp, qfsvfsp->vfs_fs);
#endif /* LUFS */
...
static void
logmap_cancel_delta(ml_unit_t *ul, offset_t mof, uchar_t ord, int32_t nb,
			int metadata)
{
	mapentry_t	*me;
	mapentry_t	**mep;
	...
	if (metadata) {
		...
		LQFS_MSG(CE_WARN, "logmap_cancel_delta(): Wrote map entry "
		    "mof 0x%x ord %d nb %ld to on-disk log\n",
		    me->me_mof, me->me_ord, me->me_nb);
	}
	 ...
	 */
	LQFS_MSG(CE_WARN, "logmap_cancel_delta(): Put in map hash "
	    "chain %d entry mof 0x%x ord %d\n",
	    (MAP_ORD_OFFSET(ord, mtm) + MAP_INDEX(mof, mtm)),
	    me->me_mof, me->me_ord);
	mep = MAP_HASH(mof, ord, mtm);
	mutex_enter(&mtm->mtm_mutex);
	...
	*mep = me;
	...
	if (metadata) {
		mtm->mtm_nme++;
		mtm->mtm_nmet++;
		LQFS_MSG(CE_WARN, "logmap_cancel_delta(): Added metadata "
		    "mof 0x%x ord %d nb %d (nme %d)\n", me->me_mof, me->me_ord,
		    me->me_nb, mtm->mtm_nme);
	} else {
		me->me_flags = ME_USER;
		LQFS_MSG(CE_WARN, "logmap_cancel_delta(): Added user mof 0x%x "
		    "ord %d nb %ld to logmap (nme %d)\n", me->me_mof,
		    me->me_ord, me->me_nb, mtm->mtm_nme);
	}
	me->me_flags |= (ME_HASH|ME_CANCEL);
	if (!(metadata)) {
		frags = blkoff(ul->un_qfsvfs->vfs_fs, nb);
		if (frags) {
			mtm->mtm_cfrags += numfrags(ul->un_qfsvfs->vfs_fs,
			    frags);
		}
	}
	mutex_exit(&mtm->mtm_mutex);
 ...
 */
void
logmap_cancel(ml_unit_t *ul, offset_t mof, uchar_t ord, off_t nb, int metadata)
{
	int32_t		hnb;
	mapentry_t	*me;
	mapentry_t	**mep;
	...
	for (hnb = 0; nb; nb -= hnb, mof += hnb) {
		 ...
		 * find overlapping entries
		 */
		LQFS_MSG(CE_WARN, "logmap_cancel(): Cancelling map hash "
		    "chain %d entries that are within mof 0x%x ord %d "
		    "nb %d\n", (MAP_ORD_OFFSET(ord, mtm) + MAP_INDEX(mof, mtm)),
		    mof, ord, hnb);
		mep = MAP_HASH(mof, ord, mtm);
		mutex_enter(&mtm->mtm_mutex);
		for (me = *mep; me; me = me->me_hash) {
			...
			ASSERT(MEwithinDATA(me, mof, ord, hnb));
			...
			if ((me->me_flags & ME_CANCEL) == 0) {
				...
				found++;
				LQFS_MSG(CE_WARN, "logmap_cancel(): "
				    "Put mapentry mof 0x%x ord %d nb %ld "
				    "on cancel list.\n", me->me_mof,
				    me->me_ord, me->me_nb);
			}
		}
		 ...
		 */
		if (found) {
			logmap_cancel_delta(ul, mof, ord, hnb, metadata);
		}
	}
 ...
 */
int
logmap_iscancel(mt_map_t *mtm, offset_t mof, uchar_t ord, off_t nb)
{
	off_t		hnb;
	mapentry_t	*me;
	mapentry_t	**mep;
	...
	for (hnb = 0; nb; nb -= hnb, mof += hnb) {
		 ...
		 */
		LQFS_MSG(CE_WARN, "logmap_iscancel(): Checking map hash "
		    "chain %d for cancelled entry that overlaps mof 0x%x "
		    "ord %d nb %d\n", (MAP_ORD_OFFSET(ord, mtm) +
		    MAP_INDEX(mof, mtm)), mof, ord, hnb);
		mep = MAP_HASH(mof, ord, mtm);
		for (me = *mep; me; me = me->me_hash) {
...
logmap_logscan_add(ml_unit_t *ul, struct delta *dp, off_t lof, size_t *nbp)
{
	...
	if (dp->d_typ == DT_COMMIT) {
		...
		/* ordinal within valid range */
		if (dp->d_ord > qfsvfsp->mt.fs_count) {
			error = EINVAL;
		}
	}

	if (error) {
		ldl_seterror(ul, "Error processing qfs log data during scan");
		return (error);
	}
...
logmap_logscan(ml_unit_t *ul)
{
	...
	ASSERT(
	    ul->un_deltamap->mtm_next == (mapentry_t *)(void *)ul->un_deltamap);
...
_init_map(void)
{
	 ...
	 */
	mapentry_cache = kmem_cache_create("lqfs_mapentry_cache",
...
handle_dquot(mapentry_t *me)
{
	 ...
	 */
	dolock = (!RW_LOCK_HELD(&dqp->dq_qfsvfsp->vfs_dqrwlock));
	if (dolock) {
		rw_enter(&dqp->dq_qfsvfsp->vfs_dqrwlock, RW_READER);
	}
	...
	if (dqp->dq_cnt == 0) {
		...
		if (dolock) {
			rw_exit(&dqp->dq_qfsvfsp->vfs_dqrwlock);
		}
		return;
	}
