 ...
 * using the list_activities function with a filter on the job id.
 */
int
add_storage_node(ctx_t *c /* ARGSUSED */, char *fs_name, char *node_name,
    char *node_ip, fs_t *backing_store, char *node_data) {

	add_storage_node_arg_t *asn;
	op_req_t req;

	/* node_data and node_ip can be null */
	if (ISNULL(fs_name, node_name, backing_store)) {
		Trace(TR_ERR, "add storage node failed: %s",
		    samerrmsg);
		return (-1);
	}
	asn = (add_storage_node_arg_t *)mallocer(
		sizeof (add_storage_node_arg_t));
	if (asn == NULL) {
		Trace(TR_ERR, "add storage node failed: %s",
		    samerrmsg);
		return (-1);
	...
	asn->ctx = (ctx_t *)mallocer(sizeof (ctx_t));
	if (asn->ctx == NULL) {
		Trace(TR_ERR, "add storage node failed: %s",
		    samerrmsg);
		return (-1);
	...
	asn->ctx->handle = NULL;
	asn->ctx->dump_path[0] = '\0';
	asn->ctx->read_location[0] = '\0';
	asn->ctx->user_id[0] = '\0';

	asn->fs_name = copystr(fs_name);
	if (asn->fs_name == NULL) {
		free(asn);
		Trace(TR_ERR, "add storage node failed: %s",
		    samerrmsg);
	...
	asn->node_name = copystr(node_name);
	if (asn->node_name == NULL) {
		free_dispatch_func_args(asn, CMD_ADD_SOSD);
		Trace(TR_ERR, "add storage node failed: %s",
		    samerrmsg);
		return (-1);
	}

	/* asn->node_ip can be NULL if the input was */
	asn->node_ip = copystr(node_ip);
	if (node_ip && asn->node_ip == NULL) {
		free_dispatch_func_args(asn, CMD_ADD_SOSD);
		Trace(TR_ERR, "add storage node failed: %s",
		    samerrmsg);
		return (-1);
	...
	/* asn->node_data can be NULL if the input was */
	asn->node_data = copystr(node_data);
	if (node_data && asn->node_data == NULL) {
		free_dispatch_func_args(asn, CMD_ADD_SOSD);
		Trace(TR_ERR, "add storage node failed: %s",
		    samerrmsg);
		return (-1);
	...
	/*
	 * build a request- this can be a local variable because this
	 * request is done synchronously
	 */
	req.func_id = CMD_ADD_SOSD;
	req.job_id = NULL;
	req.host_num = NULL;
	req.args = asn;

	/* send the request to the storage node synchronously */
	if (send_request(&req, node_name) != 0) {
		Trace(TR_ERR, "add_storage node failed: %d %s",
		    samerrno, samerrmsg);
		free_dispatch_func_args(asn, CMD_ADD_SOSD);
		return (-1);
	}

	/* tear down the args to send_request */
	free_dispatch_func_args(asn, CMD_ADD_SOSD);

	/*
	 * once osd config code is available setup the initiator and
...
char *fs_name,
char *node_name,
char *node_ip /* ARGSUSED */,
fs_t *fs,
char *node_data /* ARGSUSED */) {


	if (ISNULL(fs_name, node_name, fs)) {
		Trace(TR_ERR, "Adding the sosd failed: %d %s",
		    samerrno, samerrmsg);
		return (-1);
	}
	...
	/*
	 * If the backing store does not have a name use the fs_name
	 * for which it is a part.
	 */
	if (*fs->fi_name == '\0') {
		strlcpy(fs->fi_name, fs_name, sizeof (uname_t));
	}

	if (*fs->fi_mnt_point == '\0') {
		*fs->fi_mnt_point = '/';
		strlcat(fs->fi_mnt_point, fs->fi_name, sizeof (upath_t));
	}

	/* Create the backing store */
	if (create_fs_and_mount(c, fs, B_TRUE, B_TRUE, B_TRUE) != 0) {
		return (-1);
	}

	/* Once osd config code is available create the osd device here */
...
char *fs_name) {
	...
	/* Once the osd configuration code is available set up the proto fs. */
	if (proto_fs == NULL) {
...
get_shared_fs_summary_status(ctx_t *c, char *fs_name, sqm_lst_t **lst) {
	...
	*lst = lst_create();
	...
	snprintf(buf, sizeof (buf), "storage_nodes=%d,unmounted=0,"
	    "error=0,off=0", proto_nodes);
	str = copystr(buf);
	if (str == NULL) {
		lst_free_deep(*lst);
		Trace(TR_ERR, "get shared fs summary status failed: %s",
