/* 
 * GNU du from fileutils-3.16
 * du from fileutils-3.15 fixed for samfs:
 *  - a released file reports st_blocks as 0. in that case, we use
 *    (st_size+511)/512
 *  - size, tot_size and some routines had to be changed to u_longlong_t, also
 *    some fprintf formats now use %lld instead of %ld
 *
 * Andi Karrer, 29-Nov-1996
 *
 * updated for fileutils-3.16
 * Harold Melanson, 27-Mar-2000
 */
...
static u_longlong_t count_entry __P ((char *ent, int top, dev_t last_dev));
...
static u_longlong_t tot_size = (u_longlong_t)0;
...
static char *
human_readable (u_longlong_t n_bytes, char *buf, int buf_len)
{
  const char *suffix;
  double amt;
  char *p;
...
count_entry (char *ent, int top, dev_t last_dev)
{
  ...
  if (S_ISLNK(stat_buf.st_mode) && xstat != L_STAT) {
      size = 0;				/* Symbolic link and no -L opt, skip it */
  } else {
    if (sam_stat(ent, &sb, sizeof(sb))) {
      error (1, errno, "%s", path->text);
    }

    if (SS_ISREMEDIA(sb.attr) == 0) {
      if (output_size == size_bytes) {
        size = (u_longlong_t)stat_buf.st_size;
      } else {
        size = (u_longlong_t)ST_NBLOCKS(stat_buf);
        /* samfs returns 0 for st_blocks when a file is offline.
		 * If the file is segmented, some or all segments may be
		 * offline even though the segment index is online.
         * In those cases, we want the size, rounded by blocks,
         * even if it is a file with holes.
         */
        if (SS_ISOFFLINE(sb.attr) || SS_ISSEGMENT_F(sb.attr)) {
          size = (u_longlong_t)(stat_buf.st_size+511)/512;
        }
      }
    } else {
      size = 0;
