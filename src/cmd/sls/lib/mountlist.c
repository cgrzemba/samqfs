...
#if !(defined(RHE_LINUX) && (KERNEL_MAJOR == 4) && (KERNEL_MINOR < 19))
char *strstr ();
#endif
char *xmalloc ();
char *xrealloc ();
...
#ifdef linux
#include <mntent.h>	/* declare MNTTAB */
#endif /* linux */
#ifdef sun
     ...
     int need_fs_type, all_fs;
{
...
#if defined (MOUNTED_FREAD) || defined (MOUNTED_FREAD_FSTYP) /* SVR[23].  */
  {
	...
	me->me_devname = xstrdup (mnt.mnt_fsname);
#else
	me->me_devname = xmalloc (strlen (mnt.mnt_fsname) + 6);
	strcpy (me->me_devname, "/dev/");
	strcpy (me->me_devname + 5, mnt.mnt_fsname);
#endif
	me->me_mountdir = xstrdup (mnt.mnt_dir);
	me->me_dev = -1;	/* Magic; means not known yet. */
	me->me_type = "";
