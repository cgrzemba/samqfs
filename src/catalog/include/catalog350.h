...
#define	CF_VERSION 350		/* Increment whenever catalog format changes */
...
struct CatalogEntry {
	...
	union {		/* Media dependent */
		uint64_t CePtocFwa;	/* Opticala: First word addr of PTOC */
...
struct CatalogHdr {
	...
	char	ChMediaType[3];		/* Media type */
	uint16_t ChEq;			/* Equipment number */
	struct CatalogEntry ChTable[1];	/* Catalog table - Catalog entry(s) */
};

#endif /* defined(CATALOG_H) */
